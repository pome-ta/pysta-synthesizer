...
class Oscillator:
  def __init__(self):
    self.amplitude = 1
    self.frequency = 440
    self.wave_box = [self.mixwave, self.sine, self.triangle, self.sawtooth, self.square, self.whiteNoise]
...
class Synth:
  ...
  def set_up(self):
    ...
    inputFormat = AVAudioFormat.alloc().initWithCommonFormat_sampleRate_channels_interleaved_(
    ...
    mainMixer.installTapOnBus_bufferSize_format_block_(0, 64 * 64, inputFormat, self.tap_block)
    audioEngine.prepare()
    self.audioEngine = audioEngine

  def SourceNodeRender(self, _cmd, isSilence_ptr, timestamp_ptr, frameCount_ptr, outputData_ptr):
    ...
    for frame in range(frameCount_ptr):
      ...
      for bufferr in range(ablPointer.mNumberBuffers):
        buffer = ctypes.cast(ablPointer.mBuffers[bufferr].mData, ctypes.POINTER(ctypes.c_float * frameCount_ptr)).contents
